...
<div class="questions"  id="question">
	<?  echo "<pre>".print_r($arResult["ITEMS"][0]["PROPERTIES"]["QUESTION"], 1)."</pre>";
	foreach($arResult["ITEMS"] as $arItem):?>

	<span class="title"><span class="number"><?=$arResult["NAV_RESULT"]->NavPageNomer?>.</span><?=$arItem["NAME"]?></span>
	<ul class="answer">
		<? foreach($arItem["PROPERTIES"]["QUESTION"]["VALUE"] as $QuestionValue):?>
			<li><input type="checkbox" class="checkbox" name="answer[]"><?=$QuestionValue?></li>
		<? endforeach;?>
	</ul>
	<? endforeach;?>
	<? if($arParams["DISPLAY_BOTTOM_PAGER"]):?>
		<?=$arResult["NAV_STRING"]?>
	<? endif;?>
	Вы ответили на <span class="answer_complite"><?=$arResult["NAV_RESULT"]->NavPageNomer-1?></span> из <?=$arResult["NAV_RESULT"]->NavPageCount?> вопросов
</div>
