...
?>
<input type="hidden" name="IBLOCK_SECTION[]" value="<?echo IntVal($IBLOCK_SECTION_ID)?>">
...
if(count($PROP)>0):
	...
	foreach($PROP as $prop_code=>$prop_fields):
		...
		<tr id="tr_PROPERTY_<?echo $prop_fields["ID"];?>">
			...
			<td>
			<table cellpadding="0" cellspacing="0" border="0" class="nopadding" width="100%" id="tbd126720e24319e55fc156dc507f1dbd1">
			<tbody>
			<? foreach($prop_values as $k=>$val) { ?>
			<tr><td>
				<input name="PROP[<?=$prop_fields['ID']?>][<?=$k?>][VALUE]" value="<?=$val['VALUE']?>" size="30" type="text">
				<span title="Описание значения свойства">
					Описание:
					<select name="PROP[<?=$prop_fields['ID']?>][<?=$k?>][DESCRIPTION]">
						<option value=""></option>
						<option <?=($val['DESCRIPTION'] == 'Правильно' ? 'selected="selected"' : '')?> value="Правильно">Правильно</option>
					</select>
					<? /*<input name="PROP[<?=$prop_fields['ID']?>][<?=$k?>][DESCRIPTION]" value="<?=$val['DESCRIPTION']?>" size="18" type="text" id="PROP[60][182][DESCRIPTION]">*/?>
				</span>
				<br>
			</td></tr>
			<? } ?>
			<? for($i=1;$i<5;$i++) { ?>
			<tr><td>
				<input name="PROP[<?=$prop_fields['ID']?>][n<?=$i?>][VALUE]" value="" size="30" type="text">
				<span title="Описание значения свойства">
					Описание:
					<? /*<input name="PROP[<?=$prop_fields['ID']?>][n<?=$i?>][DESCRIPTION]" value="" size="18" type="text">*/ ?>
					<select name="PROP[<?=$prop_fields['ID']?>][n<?=$i?>][DESCRIPTION]">
						<option value=""></option>
						<option value="Правильно">Правильно</option>
					</select>
				</span>
				<br>
			</td></tr>
			<? } ?>
			<tr><td><input type="button" value="Добавить" onclick="addNewRow('tbd126720e24319e55fc156dc507f1dbd1')"></td></tr>
			<script type="text/javascript">BX.addCustomEvent('onAutoSaveRestore', function(ob, data) {for (var i in data){if (i.substring(0,9)=='PROP[60]['){addNewRow('tbd126720e24319e55fc156dc507f1dbd1')}}})</script>
			</tbody>
			</table>
			</td>
		</tr>
